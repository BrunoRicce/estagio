...
const create = async (titulo) => {//{ Nome, RA, Senha, Telefone, Email, Endereco }
    try {
        const result = await connection.execute(
            ...
            [titulo.Editora, titulo.Titulo, titulo.Anop, 0, 0]
        );
        console.log("ID do inserido: "+ result[0].insertId);
...
const alter = async (titulo, id) => {//{ Nome, RA, Senha, Telefone, Email, Endereco }
    try {
       const result = await connection.execute(
           'update titulo set Id_Editora=?, Titulo=?, Ano_Publicacao=? where Id_Titulo=?',
           [titulo.Editora, titulo.Titulo, titulo.Anop, id]
       );
       await connection.execute(
           'INSERT INTO autor_do_titulo (Id_Autor, Id_Titulo) VALUES (?,?)',
           [titulo.Autor, id]
...
const alterQtd = async (qtdt, qtdd, id) => {//{ Nome, RA, Senha, Telefone, Email, Endereco }
    try {
       const result = await connection.execute(
