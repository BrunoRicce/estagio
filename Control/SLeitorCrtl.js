...
const getAll = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            ...
            // await count(alunos)
            // console.log(aluno)
            // console.log(qtd_lidos)

            return res.status(200).render('SLeitor/SLeitor', { alunos, list, login, erro });
        }
...
async function count(lalunos) {
    let exes = [], mes_atu, count, mes_bd;

    mes_atu = Number(String(hoje.getMonth() + 1).padStart(2, '0'))

    for (let i = 0; i < lalunos.length; i++) {
        count = 0;
        exes = await ee.getByAlunoId(lalunos[i].Id_Aluno)

        for (let j = 0; j < exes.length; j++) {
            mes_bd = Number(exes[j].Dt_Encerramento.getMonth() + 1)

            if (mes_bd == mes_atu)// mes_atu
                count++;
        }
        if (count > qtd_lidos) {
            qtd_lidos = count;
...
const gerarPdf = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            let path = 'MelhorLeitorMensal.pdf'
            let doc = new PDFDocument({ margin: 50 });

            await count(alunos)//alimentar a list com os exemplares do aluno
            ...
            if (aluno == null)
                ...
                console.log(aluno)
                list = await ee.getByAlunoId(aluno.Id_Aluno);
                list = await listFinal(list);
            }


            generateHeader(doc);
            if (aluno != null)
                generateTableRow(doc, 110, 'Aluno', aluno.Nome);
            else
                generateTableRow(doc, 110, 'Não exeistem leitores este mês');

            generateTableRow(doc, 140, 'Tombo', 'Titulo', 'Dt Empretimo', 'Dt Encerramento', 'Dt Devolução');
            generateInvoiceTable(doc);

            doc.end();
            doc.pipe(fs.createWriteStream(path));
            return res.status(200).render('SLeitor/SLeitorLink', { login, exe_emp });
        }
    } catch (error) {
...
function generateInvoiceTable(doc) {// a cada 20 itens add uma page
    let i, invoiceTableTop = 140, rowcont = 0;
    let position;
