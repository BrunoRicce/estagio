...
const getAll = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            ...
            return res.status(200).render('FRProfessores/FRProfessores', { list, laut, ledi, lasu, login, anse });
        }
...
const create = async (req, res) => {
    ...
    let p = await prof.getById(login.ID)
    if (dt2 >= dt) {
        requi.create(p[0], req.body.ID, req.body.Anoserie, '', req.body.dtAtu, req.body.dtDevu)
    }
    else {
        erro = "Data inválida"
        return res.status(200).render('FRProfessores/FRProfessoresInfo', { list, laut, ledi, lasu, titulo, login, hoje, erro, anse });
    }
...
const pesq = async (req, res) => {
    ...
    list = await merge(list);
    return res.status(200).render('FRProfessores/FRProfessores', { list, laut, ledi, lasu, login });
}
