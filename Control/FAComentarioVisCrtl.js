...
const getAll = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            ...
            list = await comen.getAllAprovado();

            list = await merge(list)
            return res.status(200).render('FAComentarioVis/FAComentario', { list, login });
        }
...
const getById = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            ...
            return res.status(200).render('FAComentarioVis/FAComentarioInfo', { list, comentario, login, hoje, erro, erroEstado });
        }
    } catch (error) {
        return res.status(500).render('errors/error', { error: ' ERROR 500' });
    }
...
const create = async (req, res) => {
    erroEstado = "";
    comen.delById(comentario[0].Id_Comentario)
...
async function merge(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        lista.push({ Id_Comentario: list[i].Id_Comentario, Titulo: ti[0].Titulo, Visivel: list[i].Visivel, Aprovado: list[i].Aprovado, Observacao: list[i].Observacao, Ano_Publicacao: ti[0].Ano_Publicacao, Dt_Publicacao: list[i].Dt_Publicacao.toLocaleDateString(), Comentario: list[i].Comentario, Id_Autor: autor[0].Id_Autor, Autor: autor[0].Nome, autor_comentario: autor_comentario[0].Nome, autor_comentario_id: id, Id_Assunto: assunto[0].Id_Assunto, Assunto: assunto[0].Nome, Id_Editora: editora[0].Id_Editora, Editora: editora[0].Nome });
    }
    return lista;
}
