...
const getAll = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            ...
            return res.status(200).render('SDevolucao/SDevolucao', { link, login });
        }
...
const gerarPdf = async (req, res) => {
    try {
        if (req.session.Acesso != undefined) {
            let path = 'DevolucoesAtrazadas.pdf'
            let doc = new PDFDocument({ margin: 50 });
            await dataAtual();

            exe_emp = await ee.getAll();
            ...
            generateHeader(doc);
            generateTableRow(doc, 110, 'Tombo', 'Titulo', 'DT Emprestimo', 'DT Devolução', 'Estado');
            ...
            console.log(exe_emp)
            return res.status(200).render('SDevolucao/SDevolucaoLink', { login, exe_emp });
        }
    } catch (error) {
...
function generateInvoiceTable(doc) {// a cada 20 itens add uma page
    ...
    for (i = 0; i < exe_emp.length; i++) {
        const item = exe_emp[i];
        if (item.Atrasado) {
            rowcont++;
            if (rowcont < 20)
